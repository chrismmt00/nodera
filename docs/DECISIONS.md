...
**036 — 2026-07-18 — UI snippets use env vars by default and create reveal-once keys only on demand.** API key plaintext is intentionally unavailable after creation (DECISIONS 028), but Phase 7.4 needs snippets that can be pasted immediately. Snippets therefore default to `NODERA_API_KEY` and never depend on stored plaintext; the "Insert key" action creates a new session-only account key through `/api/account/keys`, inserts that reveal-once value into the current browser snippet, and still stores only the hash.

**037 — 2026-07-18 — Usage aggregation is session-only and does not widen `/v1`.** Phase 7.5 needs a scalable customer usage page, but `docs/api.md` has no public aggregate endpoint and fetching every job detail from the browser would not scale. `/api/account/usage` therefore follows the same account-route pattern as API keys: signed-in workspace only, Prisma-only aggregation over current-month succeeded run rows, no API-key access, and no new public contract surface.
